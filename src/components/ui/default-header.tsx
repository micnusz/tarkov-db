...
import { ContextMenuContent, ContextMenu, ContextMenuCheckboxItem, ContextMenuTrigger } from "./context-menu";
...
export default function DefaultHeader<TData, TValue>({
    ...
    return (
        <ContextMenu>
            <ContextMenuTrigger className="flex w-full h-full justify-start items-center gap-4" 
                onPointerDown={(e) => {
                e.preventDefault(); 
            ...
            table.getAllColumns().filter((column) => column.getCanHide()).map((column) => (
                <ContextMenuCheckboxItem 
                    key={column.id} 
                    className="capitalize" checked={column.getIsVisible()} 
